...
function shell() {
	document.body.innerHTML = `
		...
		<button data-issue-trigger type="button">Report</button>
...
test("authenticated trigger replaces the external issue link", () => {
	...
	setServerUser(null);
	syncIssueReportTrigger();
	assert.equal(document.querySelector("[data-issue-trigger]").hidden, true);
...
test("drawer collects context, requires review, then publishes the approved report", async () => {
	...
	const trigger = document.querySelector("[data-issue-trigger]");
	trigger.click();
