...
test("an empty release manifest announces nothing and never takes focus", async () => {
	// The manifest ships empty and is filled in at deploy, so this is what
	// every checkout and every local run serves. Opening a panel that says
	// there is nothing to read — and pulling focus into it — is worse than
	// staying quiet.
	mountWhatsNew();
...
test("?whats-new=1 still opens the panel on an empty manifest", async () => {
	// The explicit debugging affordance has to keep working, empty state and all.
	mountWhatsNew();
