...
(function () {
	...
	const EXPERIMENTS = [
		...
		{ group: "Your contributions — saved only in this browser", items: [
			{ name: "Favorites", what: "Save tools and see them collected in one place.", sim: "A set of tool names in localStorage; the tool data is read live.", needs: "POST / DELETE /api/user/favorites/", tryHref: "#/favorites", tryLabel: "Open favorites" },
			{ name: "Lists", what: "Create, edit, reorder and delete lists, and add tools to them.", sim: "Lists of tool names in localStorage, shown alongside the live lists.", needs: "POST / PUT / DELETE /api/lists/", tryHref: "#/my-lists", tryLabel: "Your lists" },
			{ name: "Submit a tool", what: "Add a brand-new tool record.", sim: "A local record (clearly not shown in live search).", needs: "POST /api/tools/", tryHref: "#/tools/create", tryLabel: "Submit a tool" },
			{ name: "Edit a tool", what: "Change a tool's core fields (title, description, links…).", sim: "Field overrides merged onto the live record at render time.", needs: "PUT /api/tools/{name}/ and edit permissions" },
			{ name: "Edit annotations", what: "Add community annotations (audiences, tasks, type, icon).", sim: "Annotation overrides merged onto the live record.", needs: "PUT /api/tools/{name}/annotations/" },
			{ name: "Add / remove tools (crawler)", what: "Register a toolinfo.json URL, or paste / load sample toolinfo to ingest tools.", sim: "URLs recorded locally; ingestion parses pasted JSON (the browser can't fetch arbitrary URLs — CORS).", needs: "A server-side crawler", tryHref: "#/add-or-remove-tools", tryLabel: "Add or remove tools" },
			{ name: "Activity feeds", what: "Your demo edits appear at the top of Recent changes, Audit logs and tool history.", sim: "Local revision/audit rows merged on top of the live feeds.", needs: "Server-side write side-effects", tryHref: "#/recent", tryLabel: "Recent changes" },
		] },
	...
	function viewExperiments() {
		const groups = EXPERIMENTS.map((g) => `
		...
		return { title: "Experimental features — Toolhub", html: `
			<div class="container page">
	...
	function dispatch() {
		...
		if (seg[0] === "experiments") return viewExperiments();
