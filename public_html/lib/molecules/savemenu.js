		...
		demoLists()
			.map(
				/** @param {{ id: string; title?: string; tools?: string[] }} l */ (l) => {
					...
					return `<button class="savemenu__item${inIt ? " is-on" : ""}" type="button" data-listadd="${esc(l.id)}" data-tn="${esc(name)}" aria-pressed="${inIt}"><span class="savemenu__mark" aria-hidden="true">${inIt ? icon("check") : icon("add")}</span> <span${dirAttrs(l.title)}>${esc(l.title || t("savemenu.untitledList", "Untitled list"))}</span></button>`;
				}
			)
