...
export function authorProfileUrl(profile, name) {
...
export function authorIndex() {
	if (!authorIndexPromise) {
		authorIndexPromise = loadAllTools().then((tools) => {
			const index = new Map();
...
export async function toolsByAuthor(name) {
	...
	return index.get(keyOf(name)) || { name, tools: [], profile: {} };
}
