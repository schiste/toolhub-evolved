 ...
 * @param {string} salt
 */
export function synthSeed(name, salt) {
	return hash(`${name}·${salt}`);
}
// Popularity — Needs: usage/view tracking the API doesn't expose.
/** @param {string} name */
export function synthViews(name) {
	const h = hash(name),
		b = h % 100;
	return b >= 92 ? 1000 + (h % 1500) : b >= 70 ? 250 + (h % 750) : 20 + (h % 230);
}
...
/** @param {string} name */
export function synthThanks(name) {
	return 3 + (synthSeed(name, "thanks") % 140);
}
// 30-day usage — Needs: usage analytics the API doesn't expose.
/** @param {string} name */
export function synthUsage(name) {
	return 50 + (synthSeed(name, "usage") % 9000);
}
