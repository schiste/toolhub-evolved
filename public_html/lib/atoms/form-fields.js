...
export function fInput(label, id, value, opts = {}) {
	...
	// Stryker disable ConditionalExpression,LogicalOperator: the disable spans the template line — the co-disabled req/ph/hasError ternaries and type/max `||` defaults are equivalent (verified by the fInput assertions). esc() coerces null/undefined to "", so value needs no guard.
	return `<label class="le__label">${esc(label)}${opts.req && opts.reqMark !== false ? ' <span class="le__req">*</span>' : ""}
		${fHint(id, hint)}
		<input class="${fInputClass(opts)}" id="${id}" type="${opts.type || "text"}"${opts.req ? " required" : ""}${fDescAttrs(id, hint, hasError)} maxlength="${opts.max || 300}" value="${esc(value)}" ${opts.ph ? `placeholder="${esc(opts.ph)}"` : ""} />${hasError ? `<span class="le__error" id="${esc(id)}-err" hidden></span>` : ""}</label>`;
...
export function fArea(label, id, value, hint, opts = {}) {
	const fieldHint = hint ?? opts.hint;
	const maxAttr = opts.max === false ? "" : ` maxlength="${opts.max || 2000}"`;
	// Stryker disable ConditionalExpression,LogicalOperator: the disable spans the template line — the co-disabled rows/ph `||`/ternary defaults are equivalent (verified by the fArea assertions). esc() coerces null/undefined to "", so value needs no guard.
	return `<label class="le__label">${esc(label)}${fHint(id, fieldHint)}
		<textarea class="${fInputClass(opts)}" id="${id}" rows="${opts.rows || 3}"${fDescAttrs(id, fieldHint, false)}${maxAttr}${opts.ph ? ` placeholder="${esc(opts.ph)}"` : ""}>${esc(value)}</textarea></label>`;
	// Stryker restore ConditionalExpression,LogicalOperator
}
/**
 * @param {string} label
