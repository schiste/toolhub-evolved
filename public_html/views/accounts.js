...
function syncNotice(sync) {
	...
	if (status === "stale") {
		const completed = sync?.lastCompletedAt
			? ` ${t("accounts.lastComplete", "Last complete refresh: $1.", timeTag(sync.lastCompletedAt))}`
			: "";
...
function accountResults(directory, state) {
	...
	return `<section aria-labelledby="account-results-title">
		<div class="section-head people-page__results-head"><div><h2 id="account-results-title" class="people-page__section-title">${t("accounts.officialAccounts", "Official Toolhub accounts")}</h2><p class="muted" aria-live="polite">${t("accounts.range", "Showing $1–$2 of $3 accounts", first, last, directory.count)}</p></div></div>
	...
	return {
		title:
			state.accountId && detail?.username
				? t("accounts.detailDocTitle", "$1 — Accounts — Toolhub", detail.username)
