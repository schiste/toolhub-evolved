...
function relationshipSummaryLabel(role, verified) {
	...
	return verified
		? t("authors.verifiedRelationship", "Verified {relationship} relationship", { relationship: label })
		: t("authors.listedRelationship", "Listed {relationship}", { relationship: label });
}
...
function personCard(item) {
	...
	const matchDetail = matchBasis.includes("tool")
		...
		: account?.username
			? `<small${dirAttrs(account.username)}>${t("authors.accountUsername", "Toolhub username: {username}", { username: account.username })}</small>`
			: "";
...
function communityDirectoryResults(directory, state) {
	...
	const breakdown = [
		Number(counts.people) > 0
			? countLabel(counts.people, t("authors.personOne", "person"), t("authors.personOther", "people"))
			: "",
		Number(counts.accounts) > 0
			? countLabel(
					counts.accounts,
					t("authors.accountOne", "account-only result"),
					t("authors.accountOther", "account-only results")
				)
			: "",
		Number(counts.unresolvedAttributions) > 0
			? countLabel(
					counts.unresolvedAttributions,
					t("authors.unresolvedOne", "unresolved attribution"),
					t("authors.unresolvedOther", "unresolved attributions")
				)
			: ""
	].filter(Boolean);
	const range =
		results.length > 0
			? t("authors.showingCommunityRange", "Showing {first}–{last} of {count} results", { first, last, count })
			: t("authors.communityCount", "{count} results", { count });
	const cards = results
		.map((/** @type {any} */ item) => {
...
export async function viewPeople() {
	...
	return {
		title: state.q
			? t("authors.communitySearchDocTitle", "{query} — Community — Toolhub", { query: state.q })
