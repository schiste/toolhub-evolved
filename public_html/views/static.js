...
export const STATIC = {
	...
	"mcp-server": () => ({
		...
		<p>${t("static.mcp.promptWhy", "Toolforge is experiencing an influx of new developers, which is excellent. Unfortunately, the LLM tools currently in use often are too happy to reinvent the wheel. To help collaboration and reuse, developers should instruct their tools to check for existing tools and code before starting on new projects or features.")}</p>
		<p>${tWithElements("static.mcp.promptIntro", "The $1 prompt runs that check end to end. Give it an idea in a sentence or three, and it reports in three sections: build, reuse, or differentiate; adjacent tools worth extending; and a recommended stack ranked by how many existing tools already use each library.", { html: code("prior-art-review") })}</p>
