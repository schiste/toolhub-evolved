...
export const STATIC = {
	...
	"rules-of-engagement": () => ({
		...
		<p>${tWithElements("static.rulesOfEngagement.whatsRealBody", "The catalog itself is genuine: it is read live from {toolhub} through a server-side cache/proxy. That means the tools, search and facets, tool detail pages, lists, members, recent changes, crawler history, and audit logs you see are the actual Toolhub data, served quickly from Evolved's shared cache and refreshed from Toolhub according to endpoint-specific TTLs. When you sign in with Toolhub, supported writes are sent back to the official Toolhub API using your OAuth grant.", { toolhub: ext("https://toolhub.wikimedia.org/api/", "toolhub.wikimedia.org") })}</p>
