...
import { skeletonLine, tableSkeleton } from "../lib/molecules/skeleton.js";
...
/** @param {Array<{ start_date?: string, crawled_urls?: number, new_tools?: number, updated_tools?: number, total_tools?: number }>} runs */
function crawlerRows(runs) {
	if (runs.length === 0) {
		return `<tr><td colspan="5"><p class="empty">${esc(t("parity.noCrawlerRuns", "No crawler runs are available right now."))}</p></td></tr>`;
	}
	return runs
		.map(
			(r) => `
...
/** @param {Array<{ start_date?: string, crawled_urls?: number, new_tools?: number, updated_tools?: number, total_tools?: number }>} runs */
function crawlerContent(runs) {
	const last = runs[0] || {};
	const rows = crawlerRows(runs);
...
function crawlerLoadingContent() {
	const loading = `<span aria-hidden="true">${skeletonLine("skeleton--w-sm")}</span>`;
	return `<div class="detail__meta">
		${metaItem(t("parity.lastCrawl", "Last crawl"), loading)}
		${metaItem(t("parity.urlsCrawled", "URLs crawled"), loading)}
		${metaItem(t("parity.updatedInLastRun", "Updated in last run"), loading)}
	</div>
	<section class="crawler-graph crawler-graph--empty" aria-label="${esc(t("parity.crawlerRunGraph", "Crawler run graph"))}">
		${skeletonLine("skeleton-page__title skeleton--w-md")}
		${skeletonLine("skeleton-page__intro skeleton--w-xl")}
	</section>
	${tableSkeleton({ columns: 5, rows: 5, wrapClass: "crawler-history__table-skeleton", tableClass: "runs" })}`;
}

// Crawler history — cached official data from /api/crawler/runs/.
export function viewCrawler() {
	return {
		...
		mount() {
			apiGet("/crawler/runs/", { page_size: "12" })
				.then((data) => {
					const target = document.querySelector("[data-crawler-history-content]");
					if (!target) return;
					const runs = Array.isArray(data?.results) ? data.results : [];
					target.innerHTML = crawlerContent(runs);
					target.setAttribute("aria-busy", "false");
				})
				.catch(() => {
					const target = document.querySelector("[data-crawler-history-content]");
					if (!target) return;
					target.innerHTML = crawlerContent([]);
					target.setAttribute("aria-busy", "false");
				});
		}
	};
