...
function activeClientStatuses(value) {
	return new Set(String(value || "").split(",").map((s) => s.trim()).filter((s) => CLIENT_STATUS_VALUES.has(s)));
}
...
export async function viewSearch() {
	...
	if (clientStatuses.size) results = results.filter((t) => CLIENT_STATUS_FILTERS.some((s) => clientStatuses.has(s.value) && s.match(t)));
	if (sort === "views") results.sort((a, b) => (b.weeklyViews - a.weeklyViews) || a.title.localeCompare(b.title));
	if (sort === "complete") results.sort((a, b) => (completeness(b).filled - completeness(a).filled) || a.title.localeCompare(b.title));
	...
	const countHTML = clientStatuses.size
		? (results.length
	...
	const countNoteHTML = clientStatuses.size ? ' <span class="browse__count-note">filtered in your browser</span>' : "";
	...
	function mount() {
		...
		const navigate = (extra) => {
			...
			$$(".facets input[type=checkbox][data-facet]:checked").forEach((c) => u.append(c.getAttribute("data-facet"), c.value));
			const statuses = $$(".facets input[type=checkbox][data-client-status]:checked").map((c) => c.getAttribute("data-client-status"));
			if (statuses.length) u.set("status", statuses.join(","));
