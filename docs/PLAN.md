...
| **Add / remove tools**          | Paste a homepage or public `toolinfo.json` URL, or paste toolinfo for Evolved-local ingestion.                 | Evolved discovers root/sitemap `toolinfo.json` URLs before official-first registration and indexes official crawler feed sources.   | crawler URL discovery + source index + feed         |
...
| **My tools**                    | See official Toolhub tools associated with the signed-in user, grouped by verified and possible author claims. | Tool rows come from official Toolhub; verification, official feed source, and root/sitemap discovery are per-tool Evolved evidence. | `/v1/me/tools/` + author claims + source caches     |
