...
| **My tools**                    | Review tools linked to the signed-in person's canonical relationships, paste homepage/toolinfo URLs, and manage local toolinfo submissions. | Tool rows come from the local canonical Toolhub projection; roles and trust come from the shared typed people graph.     | `/v1/me/tools/` + account bindings + relationship graph |
