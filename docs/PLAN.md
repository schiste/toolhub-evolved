...
| **Add / remove tools**          | Paste a homepage or public `toolinfo.json` URL, or paste toolinfo for Evolved-local ingestion.                 | Evolved discovers root/sitemap `toolinfo.json` URLs before official-first registration; pasted JSON stays Evolved-local.                 | crawler URL discovery + `/toolinfo.json` feed       |
...
| **My tools**                    | See official Toolhub tools associated with the signed-in user, grouped by verified and possible author claims. | Tool rows come from official Toolhub; verification claims and catalog-seeded automated toolinfo discovery are per-tool Evolved evidence. | `/v1/me/tools/` + author claims + discovery cache   |
