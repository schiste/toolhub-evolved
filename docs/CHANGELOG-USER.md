...
- That check had been failing every week for months, and not because anything was wrong. It was set to demand a perfect result the site has never achieved, so it reported the same failure whatever happened, which is the same as reporting nothing. Meanwhile eighteen files were quietly never being checked at all -- among them four of the largest pages on the site -- and a further ten pages had been switched off by a single unrelated stopwatch. The check now asks each area to hold the standard it actually reached, so a failure means something changed for the worse.
