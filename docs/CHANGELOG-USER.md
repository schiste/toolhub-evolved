...
- Tens of thousands of user scripts have no description anywhere, and no amount of further reading of wiki pages will produce one, because nobody ever wrote it down. For those, the catalogue now reads the script's own source code through Wikimedia's Lift Wing service and records a short description and a few keywords, which appear on the tool's page within the same hour.
