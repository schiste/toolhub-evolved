...
<!-- Source range: ea9baf5..b056a3af (11 commits) -->
...
- A release is no longer finished when the files are copied across. The deploy now asks the live site four questions — is it ready, does the catalogue answer, does a real feature work, is the write guard still refusing what it should refuse — and if any answer is wrong it stops and names the version to go back to. Previously the deploy reported success as soon as the copy finished, so a broken release announced itself as a good one. Reviewing this release turned up the obvious follow-on mistake, now fixed: when a deploy picked up a new version of itself partway through, the version it offered to go back to was the broken one.
