...
_TOOL_DEFINITIONS: tuple[dict[str, Any], ...] = (
    {
        ...
        "description": (
            ...
            "contributes -- served from a synchronized local replica. Covers the latest "
            "complete catalog generation without request-time upstream calls. A tool "
            "matches when ANY of your words appears in its name, title, keywords or "
            "description; tools containing every word rank first, then a word as the "
            "whole title outranks the same word in keywords, which outranks it in the "
            "description. Plain plurals match their singular ('pageviews' finds "
            "'pageview'); there is no other stemming or fuzzy matching. Keep queries "
            "SHORT and distinctive (2-3 content words), since a merely plausible word "
            "('wikipedia', 'tool') widens the tail without changing the top results, "
            "and read the first page rather than the count."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
_PRIOR_ART_PROMPT = (
    ...
    "1. **search_tools(query, limit=10)**: Relevance-ranked word search across the "
    "full catalog of Wikimedia tools, gadgets and user scripts. Any word can match; "
    "tools containing every word rank first, and a word in the title outranks the "
    "same word in the description. Keep queries short and distinctive (2-3 content "
    "words), run several with different vocabulary, and judge by the first page "
