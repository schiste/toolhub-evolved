...
def _visibility(value: Any, default: str = common.VISIBILITY_PRIVATE) -> str:  # noqa: ANN401
    return value if value in {common.VISIBILITY_PRIVATE, common.VISIBILITY_PUBLIC} else default
...
def v1_overlay_get() -> Response:
    ...
    return jsonify(common.assemble_overlay(uid))
...
def _put_lists(uid: int, value: Any) -> Response | None:  # noqa: ANN401
    ...
    with db.session_scope() as s:
        ...
        for item in value[: v1.MAX_ITEMS]:
            ...
            last_synced_at = common.parse_optional_iso(common.payload_value(item, "lastSyncedAt", "last_synced_at"))
            if sync_status == SYNC_OFFICIAL and last_synced_at is None:
                last_synced_at = common.parse_iso(item.get("modified"))
            response = common.payload_value(item, "toolhubResponse", "last_toolhub_response")
            validation_errors = common.payload_value(item, "validationErrors", "validation_errors")
            s.add(
                ToolList(
                    ...
                    source=clean_source(
                        common.payload_value(item, "source"),
                        SOURCE_OFFICIAL if official_id else SOURCE_LOCAL,
                    ),
                    sync_status=sync_status,
                    last_synced_at=last_synced_at,
                    last_error=clean_error(common.payload_value(item, "lastError", "last_error")),
...
def _put_crawler_urls(uid: int, value: Any) -> Response | None:  # noqa: ANN401
    ...
    with db.session_scope() as s:
        ...
        for u in value[: v1.MAX_ITEMS]:
            ...
            last_synced_at = common.parse_optional_iso(common.payload_value(u, "lastSyncedAt", "last_synced_at"))
            if sync_status == SYNC_OFFICIAL and last_synced_at is None:
                last_synced_at = common.parse_iso(u.get("added"))
            response = common.payload_value(u, "toolhubResponse", "last_toolhub_response")
            validation_errors = common.payload_value(u, "validationErrors", "validation_errors")
            s.add(
                CrawlerUrl(
                    user_id=uid,
                    created_by_user_id=uid,
                    url=str(u["url"])[:2000],
                    added_at=common.parse_iso(u.get("added")),
                    official_crawler_url_id=official_id,
                    source=clean_source(
                        common.payload_value(u, "source"), SOURCE_OFFICIAL if official_id else SOURCE_LOCAL
                    ),
                    enabled=bool(u.get("enabled", True)),
                    last_checked_at=common.parse_optional_iso(
                        common.payload_value(u, "lastCheckedAt", "last_checked_at")
                    ),
                    last_status=str(common.payload_value(u, "lastStatus", "last_status") or "")[:64] or None,
                    last_error=clean_error(common.payload_value(u, "lastError", "last_error")),
                    last_toolhub_response=response if isinstance(response, dict) else None,
                    validation_errors=validation_errors if isinstance(validation_errors, list) else None,
                    sync_status=sync_status,
                    last_synced_at=last_synced_at,
...
def _valid_map(value: Any) -> bool:  # noqa: ANN401
    return isinstance(value, dict) and all(
        isinstance(k, str) and 0 < len(k) <= common.MAX_NAME and isinstance(v, dict) for k, v in value.items()
    )
...
def _tool_record_meta(
    ...
    *,
    ...
    visibility = _visibility(
        common.payload_value(rec, "visibility"),
        common.VISIBILITY_PUBLIC if is_crawler_record else common.VISIBILITY_PRIVATE,
    )
    preserved_review = clean_review_status(existing_review_status, REVIEW_PENDING)
    if can_review:
        review_status = clean_review_status(
            common.payload_value(rec, "reviewStatus", "review_status"), preserved_review
        )
    elif visibility == common.VISIBILITY_PUBLIC and not record_changed:
        review_status = preserved_review
    else:
        review_status = REVIEW_PENDING
    default_status = SYNC_EVOLVED_REAL if visibility == common.VISIBILITY_PUBLIC else SYNC_LOCAL_DRAFT
    status = clean_sync_status(common.payload_value(rec, "syncStatus", "sync_status"), default_status)
    official_name = common.payload_value(rec, "officialName", "official_name")
    response = common.payload_value(rec, "toolhubResponse", "last_toolhub_response")
    validation_errors = common.payload_value(rec, "validationErrors", "validation_errors")
    return {
        "visibility": visibility,
        "source": SOURCE_LOCAL,
        "sync_status": status,
        "review_status": review_status,
        "last_synced_at": common.parse_optional_iso(common.payload_value(rec, "lastSyncedAt", "last_synced_at")),
        "last_error": clean_error(common.payload_value(rec, "lastError", "last_error")),
        "official_name": str(official_name)[: common.MAX_NAME]
...
def _overlay_meta(
    ...
    *,
    ...
    field_statuses = common.payload_value(patch, "fieldStatuses", "field_statuses")
    response = common.payload_value(patch, "toolhubResponse", "last_toolhub_response")
    validation_errors = common.payload_value(patch, "validationErrors", "validation_errors")
    preserved_review = clean_review_status(existing_review_status, REVIEW_OPEN)
    return {
        "base_revision": str(common.payload_value(patch, "baseRevision", "base_revision") or "")[: common.MAX_NAME]
        or None,
        "field_statuses": field_statuses if isinstance(field_statuses, dict) else None,
        "source": clean_source(common.payload_value(patch, "source"), SOURCE_LOCAL),
        "sync_status": clean_sync_status(
            common.payload_value(patch, "syncStatus", "sync_status"),
            SYNC_LOCAL_FALLBACK if common.payload_value(patch, "lastError", "last_error") else SYNC_LOCAL_DRAFT,
        ),
        "last_synced_at": common.parse_optional_iso(common.payload_value(patch, "lastSyncedAt", "last_synced_at")),
        "last_error": clean_error(common.payload_value(patch, "lastError", "last_error")),
        "last_toolhub_response": response if isinstance(response, dict) else None,
        "validation_errors": validation_errors if isinstance(validation_errors, list) else None,
        "review_status": (
            clean_review_status(common.payload_value(patch, "reviewStatus", "review_status"), preserved_review)
            if can_review
...
def _put_tool_new(uid: int, entries: dict[str, dict], s: Any, *, can_review: bool = False) -> Response | None:  # noqa: ANN401
    ...
    for name, rec in entries.items():
        ...
        clean = common.clean_tool_record(rec)
        if clean is None:
            return common.bad(f"toolNew record '{name}' needs title, description and an https url")
...
def _put_tool_map(uid: int, value: Any, *, key: str, user: User) -> Response | None:  # noqa: ANN401
    ...
    with db.session_scope() as s:
        ...
        own = {n: patch for n, patch in entries.items() if others.get(n) != common.data_patch(patch)}
        existing_own = {
            r.tool_name: r
            for r in s.execute(
                select(ToolOverlay).where(ToolOverlay.kind == kind, ToolOverlay.user_id == uid)
...
def _put_feed(uid: int, value: Any, *, key: str) -> Response | None:  # noqa: ANN401
    ...
    if not ok:
        return common.bad(f"{key} must be a list of rows with string ids")
    with db.session_scope() as s:
            ...
            if row["id"] not in known:
                ...
                s.add(
                    ActivityRow(
                        ...
                        created_by_user_id=uid,
                        row=row,
                        created_at=common.parse_iso(row.get("timestamp")),
                    )
                )
...
def v1_overlay_put(key: str) -> Response:
    ...
    if value is None:
        return common.bad("body must be JSON")
    if key == "favorites":
    ...
    elif key in common.OVERLAY_KINDS or key == "toolNew":
        err = _put_tool_map(uid, value, key=key, user=user)
    elif key in common.FEED_KEYS:
