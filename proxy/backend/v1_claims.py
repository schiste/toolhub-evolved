...
def v1_claim_verify(claim_id: int) -> Response:  # noqa: C901, PLR0911
    ...
    with db.session_scope() as s:
        ...
        row = s.execute(
            select(ToolAuthorClaim).where(ToolAuthorClaim.id == claim_id, common.author_claim_owned_by(user))
        ).scalar_one_or_none()
        if stored_user is None or row is None:
            return common.deny(common.HTTP_NOT_FOUND, "claim not found")
        if row.revoked_at is not None:
            return common.deny(common.HTTP_CONFLICT, "revoked claims cannot be verified")
        method = row.verification_method
        if method == AUTHOR_CLAIM_AUTHOR_DISPLAY_NAME:
            return common.deny(common.HTTP_CONFLICT, "a listed author name is identity evidence and cannot self-verify")
        if method == AUTHOR_CLAIM_TOOLHUB_WRITE_ACCESS:
            return common.deny(common.HTTP_CONFLICT, "Toolhub record authority is verified automatically")
        if method == AUTHOR_CLAIM_TOOLINFO_URL_CONTROL:
            ...
            if challenge is None or challenge.user_id != stored_user.id:
                return common.deny(common.HTTP_NOT_FOUND, "claim challenge not found")
            updated, error = common.verify_control_challenge_record(s, stored_user, challenge)
            if error is not None:
                return error
            assert updated is not None  # noqa: S101 - helper returned no error
            return jsonify(
                {"ok": True, "claim": common.claim_payload(updated), "challenge": challenge_payload(challenge)}
            )
        tool, error = common.claim_tool_or_error(row.tool_name)
        if error is not None:
            return error
        ...
        return jsonify({"ok": True, "claim": common.claim_payload(updated)})
...
def v1_claim_revoke(claim_id: int) -> Response:
    ...
    with db.session_scope() as s:
        ...
        return jsonify({"ok": True, "claim": common.claim_payload(row)})
