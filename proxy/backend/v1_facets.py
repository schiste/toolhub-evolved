...
# The discovery request vocabulary, including stable aliases, is owned by one
# boundary module. In particular, legacy ``technology`` remains detected while
# declared metadata is exposed unambiguously as ``declared_technology``.
FILTER_PARAMS = facet_names.DISCOVERY_PUBLIC_TO_STORAGE
DEFAULT_LIMIT = 25
...
def _public_matched(matched: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Translate storage fields to canonical discovery response names."""
    public_rows = []
    for row in matched:
...
def normalized_filters(
    ...
    for param, facet_type in FILTER_PARAMS.items():
        ...
        filters[facet_type] = sorted(set(filters.get(facet_type, [])) | set(cleaned))
...
def v1_facets_values() -> Response | tuple[Response, int]:
    ...
    if facet_type is None:
        return jsonify({"error": f"type must be one of {sorted(FILTER_PARAMS)}"}), 400
